...
function save_one(fields,callback){
	var insert_string = modelHelper.fetchinsert("AIRPLANES",fields);
...
function save_bulk(arr_fields,callback,batchSize){
	...
	for(var i = 0 ; i < arr_fields.length; i++){
		insert_stmts.push( modelHelper.fetchinsert("AIRPLANES",arr_fields[i]) );
	}
...
function fetch_columns(fields,condition,callback,offset,limit){
   ...
   var select_stmt = modelHelper.fetchselect("AIRPLANES",fields,null,offset,limit);
   console.log(select_stmt);
   connectionPool.query(select_stmt,function(error,rows,fields){
	   if(error){
		   callback(error);
	   }else{
		   callback(null,rows);
...
module.exports = {"save_one":save_one, "save_bulk":save_bulk, "fetch_batch":fetch_batch, "fetch_columns":fetch_columns};